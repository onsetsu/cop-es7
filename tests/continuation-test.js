...
describe('continuation', function() {
  ...
  it('halts and is able to continue', function() {
    ...
    var continuation = lively.ast.StackReification.run(code, astRegistry),
  ...
  it('halts and continues function that was returned', function() {
    ...
    var continuation2 = lively.ast.StackReification.run(func, astRegistry),
  ...
  it('halts and continues in independent functions', function() {
    ...
    if (!f1.livelyDebuggingEnabled)
      f1 = f1.stackCaptureMode(null, astRegistry);

    var continuation = lively.ast.StackReification.run(f2, astRegistry, null, { f1: f1 });
    lang.arr.last(continuation.frames()).scope.set('f1', f1);
  ...
  it('halts and continues with multiple debugger breaks', function() {
    ...
    var continuation1 = lively.ast.StackReification.run(code, astRegistry),
  ...
  it('halts and interprets arguments in resume', function() {
    ...
    var continuation = lively.ast.StackReification.run(code, astRegistry, [2, 3]),
        result = continuation.resume();
  ...
  it('can throw simple error', function() {
    ...
    var expected = { isContinuation: true },
        runResult;
    try {
      lively.ast.StackReification.run(code, astRegistry);
